...
;Now find the matching Tags and copy them...
	...
	for t = 0L, Ntag_to-1 do begin
        ...
        ;tmp = struct_To[index_To].(to)
        ;struct_assign, struct_From[index_From].(from), tmp
        ;struct_To[index_To].(to) = tmp
        ;struct_To[index_To].(to) = struct_From[index_From].(from)
        ;J. Gagne to avoid structure errors
        tmp = struct_To[index_To].(to)
        if size(tmp,/type) eq 8L then begin
          struct_assign, struct_From[index_From].(from), tmp
          struct_To[index_To].(to) = tmp
        endif else begin
          struct_To[index_To].(to) = struct_From[index_From].(from)
