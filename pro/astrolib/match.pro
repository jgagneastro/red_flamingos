pro match, a, b, suba, subb, COUNT = count, SORT = sort, epsilon=epsilon, NOWARN=nowarn
;+
; NAME:
;       MATCH
; PURPOSE:
...
;-------------------------------------------------------------------------
 ...
 nowarn = 1
 if ~keyword_set(nowarn) then $
  if ~issorted(a) or ~issorted(b) then message, ' Les vecteurs A et B doivent etre classes !'
