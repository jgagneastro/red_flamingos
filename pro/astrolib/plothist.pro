PRO plothist, arr_in, xhist,yhist, BIN=bin,  NOPLOT=NoPlot, OVERPLOT=Overplot, $
...
good = where(finite(arr_in), ngood)
if ngood eq 0 then begin
  message, ' All NaN data points', /continue
  return
endif
arr = arr_in[good]
;+
...
;			Check parameters.
 On_error,2
 compile_opt idl2
 if max(~finite(arr)) eq 1 then return
...
; desired (default for integer data)    
 ...
 if N_hist gt 1d5 then begin
  message, ' Too many nhist !', /continue
  return
 endif
