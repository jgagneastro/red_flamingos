...
; Process string parameter 
 if matches GT 0 then begin
  ...
  if histnam then $
      ...
      isstring = strmid(svalue[i],0,1) EQ "'"
      if strpos(svalue[i],':') ne -1 then isstring = 1
      if ( isstring ) then begin   ;Is it a string?
          ...
          NEXT_APOST:
                  ...
                  if endap eq -1L then endap = strpos(test,'/', next_char)
                  if endap LT 0 then $ 
