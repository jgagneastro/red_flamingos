...
; Very simple-minded sum-of-squares
  ...
  mx = max(vec, min=mn, /nan)
  mx = max(abs([mx,mn]),/nan)
  if mx EQ 0 then return, vec[0]*0.

  if mx GT agiant OR mx LT adwarf then ans = mx * sqrt(total((vec/mx)^2,/nan))$
  else                                 ans = sqrt( total(vec^2,/nan) )
...
;  prof_start = systime(1)
  ...
  acnorm = make_array(n, value=min(a,/nan)*0.)
  for j = 0L, n-1 do $
  ...
  for j = 0L, minmn-1 do begin
      ...
      if j+1 LT n then begin
          for k=j+1, n-1 do begin
              ...
              if a[j,lj] NE 0 then $
                a[j,lk] = ajk - ajj * total(ajk*ajj,/nan)/a[j,lj]
...
;  prof_start = systime(1)
  ...
  nsing = n
  wa1 = qtb
  rthresh = max(abs(r[delm]),/nan)*MACHEP0
  ...
  if nsing GE n then begin
      ...
      for j=1L, n-1 do begin   ;; Note "1" here, not zero
          sum = total(r[0:(j-1),j]*wa1[0:(j-1)],/nan)
  ...
  for j=0L, n-1 do begin
      sum = total(r[0:j,j]*qtb[0:j],/nan)
      wa1[j] = sum/diag[ipvt[j]]
  endfor
  gnorm = mpfit_enorm(wa1)
  paru  = gnorm/delta
  if paru EQ 0 then paru = DWARF/min([delta,0.1],/nan)

  ;; If the input par lies outside of the interval (parl,paru), set
  ;; par to the closer endpoint

  par = max([par,parl],/nan)
  par = min([par,paru],/nan)
  ...
  if par EQ 0 then par = max([DWARF, paru*0.001],/nan)
  temp = sqrt(par)
  wa1 = temp * diag
  ...
  if fp GT 0 then parl = max([parl,par],/nan)
  if fp LT 0 then paru = min([paru,par],/nan)

  ;; Compute an improved estimate for par
  par = max([parl, par+parc],/nan)
...
function mpfit, fcn, xall, FUNCTARGS=fcnargs, SCALE_FCN=scalfcn, $
  ...
  if st1 EQ 1 AND st2 EQ 1 then begin
      ...
      if ct GT 0 then begin
          ...
          ;printarr, lindgen(n_elements(xall)), fix(xall lt LIMITS[0,*] and LIMITED[0,*]), fix(xall gt LIMITS[1,*] and LIMITED[1,*])
          ;printarr, 
          goto, TERMINATE
      endif
      ...
      if ct GT 0 then begin
          ...
          printarr, lindgen(n_elements(xall)), fix(LIMITED[0,*] and LIMITED[1,*] and LIMITS[0,*] ge LIMITS[1,*])
          goto, TERMINATE
      endif
...
;  prof_start = systime(1)
  ...
  for j=0L, n-1 do begin
      ...
      if temp3 NE 0 then begin
          ...
          wj = wa4[j:*]
          ;; *** optimization wa4(j:*)
          wa4[j] = wj - fj * total(fj*wj,/nan) / temp3  
      endif
      fjac[j,lj] = wa1[j]
      qtf[j] = wa4[j]
  endfor
  ...
  if fnorm NE 0 then begin
      for j=0L, n-1 do begin
          ...
          if wa2[l] NE 0 then begin
              sum = total(fjac[0:j,j]*qtf[0:j],/nan)/fnorm
              gnorm = max([gnorm,abs(sum/wa2[l])],/nan)
          endif
      endfor
  endif
