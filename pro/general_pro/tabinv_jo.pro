PRO TABINV_JO, XARR_IN, X, IEFF, FAST = fast
;+ 
; NAME:
;       TABINV     
; PURPOSE:  
...
;-               
 ...
 ;Repair trailing NaNs
  bad = where(~finite(xarr), nbad)
  if nbad ne 0 then begin
    lastind = max(where(finite(xarr), nf))
    if nf eq 0 then message, 'Spectra contains NaNs only !'
    xarr = xarr[0:lastind]
  endif
