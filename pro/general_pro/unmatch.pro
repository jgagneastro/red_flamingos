pro unmatch, a, b, suba, subb, nonsuba, nonsubb, COUNT = count, SORT = sort
;+
; NAME:
;       UNMATCH
; PURPOSE:
...
if ~array_equal(sort(a),lindgen(n_elements(a))) then message, 'Both array MUST be sorted ! (The first isn''t)'
if ~array_equal(sort(b),lindgen(n_elements(b))) then message, 'Both array MUST be sorted ! (The second isn''t)'
...
; Check for a single element array
 ...
 if (na EQ 1) or (nb EQ 1) then begin
        ...
        endelse
        count = nw
        nonsuba = lindgen(n_elements(a))
        nonsubb = lindgen(n_elements(b))
        if n_elements(suba) ge n_elements(nonsuba) then nonsuba = -1 else $
        if suba[0] ne -1 then remove, suba, nonsuba
        if n_elements(subb) ge n_elements(nonsubb) then nonsubb = -1 else $
        if subb[0] ne -1 then remove, subb, nonsubb
        return
 endif
