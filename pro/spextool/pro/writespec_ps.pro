...
pro writespec_ps,spectra,fullpath,aimage,sky,flat,naps,orders,$
                 ...
                 BGORDER=bgorder,TRACEFILE=tracefile,WAVECAL=wavecal, $
                 DISP=disp,FITS=fits,TEXT=text,RMS=rms,RES=res, $
                 LINEARITY=linearity,EXT2D=ext2d,CANCEL=cancel, COMPRESS=compress
...
if keyword_set(FITS) then writefits,fullpath+'.fits'+(keyword_set(compress) ? '.gz' : ''),spectra,hdr, COMPRESS=compress
