...
;  Get plot info
  ...
  max = weighted_median(spc[*,0], medval=.95)
  min = weighted_median(spc[*,0], medval=.05)
  state.p.plot1xrange = [min,max]
  max1 = weighted_median(spc[*,1], medval=.95)
  max2 = weighted_median(spc[*,2], medval=.95)
  state.p.plot1yrange = [[0,max1],[0,max2]]
  max = weighted_median(spc[*,1]/spc[*,2], medval=.95)
  min = weighted_median(spc[*,1]/spc[*,2], medval=.05)
  state.p.plot2yrange = [min,max]
  ...
  defsysv, '!SG_WINDOW', exists=sg_exists
  if ~sg_exists then sg_window = slitw_pix*scale else sg_window = !SG_WINDOW
  widget_control, state.w.sgwin_fld[1],$
                  SET_VALUE=strtrim(string(sg_window,FORMAT='(f4.1)'),2)
...
;    FWHM_kernel = sqrt(FWHM_final^2 - state.r.slitw_pix^2)
  ...
  *state.d.espec = spec
  *state.d.pspec = spec
  min = weighted_median(spec[*,1]/spec[*,2],medval=.05)
  max = weighted_median(spec[*,1]/spec[*,2],medval=.95)
  state.p.plot2yrange    = [min,max]
  xcleanspec_plotupdate,state
  xcleanspec_setminmax,state
...
pro xcleanspec_event, event
  ...
  case uvalue of
     ...
     'Plot Aperture': begin
        ...
        state.p.plot1yrange = [0,weighted_median((*state.d.espec)[*,1,idx],medval=.95)]
        sn = (*state.d.espec)[*,1,idx]/(*state.d.espec)[*,2,idx]
        min = weighted_median(sn,medval=.05)
        max = weighted_median(sn,medval=.95)
        state.p.plot2yrange = [min,max]
     ...
     case state.r.cursormode of 
        ...
        'Zoom': begin
           ...
           if count eq 0 then state.p.reg[*,0] = xy[0:1] else begin 
              ...
              min = weighted_median(state.p.reg[1,*],medval=.05)
              max = weighted_median(state.p.reg[1,*],medval=.95)
              state.p.plot1yrange   = [min,max]
              xcleanspec_plotupdate,state
              xcleanspec_setminmax,state
              state.r.cursormode   = 'None'
              state.p.reg = !values.f_nan
...
;  If not, set the keyboard focus and active window.
  ...
  if event.type eq 1 and state.r.cursormode eq 'YZoom' then begin
     ...
     if count eq 0 then begin
        ...
        state.p.reg[*,1] = xy[0:1]
        min = weighted_median(state.p.reg[1,*],medval=0.05)
        max = weighted_median(state.p.reg[1,*],medval=0.95)
        state.p.plot2yrange = [min,max]
        state.r.cursormode = 'None'
        state.p.reg = !values.f_nan
        xcleanspec_plotupdate,state
...
;******************************************************************************
;
pro xcleanspec, SCIENCE_INPUT=science_input, $
  SAVE_INPUT=save_input, SG_WINDOW=sg_window
  
  if keyword_set(sg_window) then defsysv, '!SG_WINDOW', sg_window
        ...
        col1_base = widget_base(row_base,$
           ...
           box1_base = widget_base(col1_base,$
              ...
              row = widget_base(box1_base,$
                 ...
                 dir = !NULL
                 if keyword_set(science_input) then $
                   dir = file_dirname(science_input) else begin
                     defsysv,'!spexpath',exists=existsv
                     if existsv then dir = !spexpath
                   endelse
              ...
              row = widget_base(box1_base,$
                 ...
                 inp = !NULL
                 if keyword_set(science_input) then inp = file_basename(science_input)
           ...
           state.w.box3_base = widget_base(col1_base,$
              ...
              row = widget_base(state.w.box3_base)
                 ...
                 state.w.sg_base = widget_base(row,$
                                               /ROW,$
                                               /BASE_ALIGN_CENTER)
                    if ~keyword_set(sg_window) then sg_window = 4.5
