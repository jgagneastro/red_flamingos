...
first  = strpos(strlowcase(!path),strsep,last,/REVERSE_SEARCH)
result = strmid(strlowcase(!path),first+1,last-first+7)

path = cpath(result,CANCEL=cancel)
if cancel then begin
  message, ' No Path found !', /continue
  return
endif

defsysv,'!spexpath',exists=ex

if ex eq 0 then begin
  defsysv, '!spexpath', ''
  message, ' Please define the !spexpath system variable !', /continue
endif
...
; Set default paths (Jonathan Gagne)
defsysv,'!spexpath',EXISTS=exists
if ~exists then message, ' You must define !spexpath !'
if !spexpath eq '' then message, ' You must define !spexpath !'
state.r.datapath = !spexpath
state.r.calpath = !spexpath+'cal/'
state.r.procpath = !spexpath+'proc/'
folder_check, state.r.calpath
folder_check, state.r.procpath
widget_control, state.w.datapath_fld[1], SET_VALUE=state.r.datapath
widget_control, state.w.procpath_fld[1], SET_VALUE=state.r.procpath
widget_control, state.w.calpath_fld[1], SET_VALUE=state.r.calpath
